...
export default function Merchant() {
  ...
  return (
    <>
      <div
        className={`h-[70%] lg:h-full! w-full! lg:w-[70%]! flex flex-col md:flex-row items-center gap-2 ${retroGaming.className}`}
      >
        {vendorChoices.map((item: string, index: number) => (
          ...
          >
            ...
            <p className={`text-sm! md:text-xl text-white`}>{item}</p>
          </div>
        ))}
      </div>
    </>
