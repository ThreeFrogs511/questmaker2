...
export async function PATCH(
  ...
  try {
    ...
    `;
    if (update.length <= 0)
      return NextResponse.json({ error: "error during the update" });
    ...
    if (
      ...
      if (resetLimit.length <= 0)
        return NextResponse.json({ error: "problem while updating data." });
