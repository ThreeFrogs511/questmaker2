            ...
            <div id="arena" className=" h-[80dvh]">
                <div id="healthBars" className="flex items-center h-[70%] ">
                    <div id="userSide" className="w-[80%] lg:w-[50%] mx-auto">
                        <div className="text-center flex flex-col">{currentUser.username ?? 'You'} <span>AC : {currentUser.ac}</span><span>{Math.floor(currentUser.hp ? currentUser.hp-currentUser.damage_taken : 10-currentUser.damage_taken)+'/'+currentUser.hp}</span></div>
                    ...
                    <div id="enemySide" className="w-[80%] lg:w-[50%]  mx-auto">
                        <div className="text-center flex flex-col justify-between">{enemy?.name} <span>AC : {enemy?.ac ?? 10}</span> <span>{(enemy && enemy.hp) && Math.floor(enemy.hp<=0 ? 0 : enemy.hp)+'/'+enemyFullHealth}</span></div>
