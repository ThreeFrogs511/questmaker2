      ...
      return new Promise<void>( async (resolve) => {
        ...
        if (finalDmg && encounter.hp) {
          ...
          if (!attackRoll) {
            ...
            const log = `
            <div style="margin-bottom:1rem;">
              You use <span style="color:yellow">${attack}</span> for <span style="color:yellow">${finalDmg}</span> damage !<br> 
            </div>`;
            this.setCombatLog((prev: string) => prev + log);
    ...
    async handleEnemyTurn(currentChoice:Choice, encounter:Encounter, setCurrentNode:any, node: keyof Nodes | undefined) {
      return new Promise<void>( async (resolve) => {
        ...
        if (moves && this.currentUser.hp) {
          ...
          const log = 
          `<div >
          The enemy uses <span style="color:yellow">${enemyAttack}</span> for <span style="color:yellow">${enemyFinalDmg}</span> damage !<br>
          </div>`;
          this.setCombatLog((prev: string) => prev + log);
